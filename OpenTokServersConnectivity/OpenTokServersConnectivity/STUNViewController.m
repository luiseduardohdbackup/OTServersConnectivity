...
@property (nonatomic, strong) NSDictionary * udpHosts;
@end
...
- (void) initializeAllHosts
{
    ...
    // We are assuming that in the same cluster if STUN works corresponding mantis would also work over UDP
    self.udpHosts =   @{@"turn702-mia.tokbox.com": @3478,
    ...
    [self.udpHosts enumerateKeysAndObjectsUsingBlock:^(id key, id obj, BOOL *stop) {
...
-(void) stunConnectivityChecks
{
    ...
    int numberofServers = self.udpHosts.count;
    __block int serverTested = 0;
    //make all the op asyn using operation q
    NSOperationQueue * q = [NSOperationQueue new];
    
    [self.udpHosts enumerateKeysAndObjectsUsingBlock:^(id key, id obj, BOOL *stop) {
        OTSTUNOperation * operation = [[OTSTUNOperation alloc] initWithHost:key port:[obj integerValue] timeout:10];
        __block __weak OTSTUNOperation * weakOperation = operation;
        ...
        operation.completionBlock = ^{
            ...
            if(weakOperation.connected == YES)
            {
                ...
                NSLog(@"Connected to %@ at port %d (%d/%d), stun test",key,[obj integerValue],++serverTested,numberofServers);
                
            } else {
                 [self host:key connected:NO];
                NSLog(@"Not connected to %@ at port %d (%d/%d),stun test FAILED",key,[obj integerValue],++serverTested,numberofServers);
            }
            weakOperation = nil;
